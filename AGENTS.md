...
- A normal product or metadata change must not publish. Release pushes use a `chore(release):` commit, and the workflow also supports explicit manual repair runs.
...
- GitHub release bodies must come from the matching `CHANGELOG.md` version section. Do not use autogenerated release notes.
