...
- The Worker may serve application and model bytes. It must not receive source images or perform inference.
- `build:cloudflare` must omit `public/models` and fail if any remaining static asset exceeds Cloudflare's 25 MiB per-file limit.
- Keep the standard ONNX Runtime WASM fallback. The Cloudflare build may remove only runtime artifacts that are proven unused by the accepted fallback path.
- Run `bun run cloudflare:dry-run` for web deployment changes.
