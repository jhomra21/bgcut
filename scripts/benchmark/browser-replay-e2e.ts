...
const usage =
  "Usage: bun run benchmark:browser-replay:e2e -- <manifest.json> <output-dir> <model.onnx> [port]";
...
const findBrowser = async (): Promise<BrowserTarget> => {
  ...
  throw new Error("No supported browser was found.");
};
...
const server = Bun.spawn(
  [
    "bun",
    "run",
    "scripts/benchmark/browser-replay-server.ts",
    ...args,
  ],
  {
...
while (Date.now() < startDeadline) {
  ...
  if (server.exitCode !== null) {
    throw new Error(
      `Replay server exited with code ${server.exitCode}.`,
    );
  }
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    if (await Bun.file(reportPath).exists()) {
      console.log("Replay sentinel passed.");
  ...
  if (
    ...
    throw new Error(
      "Replay sentinel timed out before completion.",
    );
  }
} finally {
  browser?.kill();
