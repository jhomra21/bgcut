...
const usage =
  "Usage: bun run benchmark:browser-reference-resume:e2e -- <manifest.json> <output-dir> <model.onnx> [port]";
...
const profileDirectory = await mkdtemp(
  join(tmpdir(), "bgcut-reference-resume-"),
);
...
while (Date.now() < startDeadline) {
  ...
  if (server.exitCode !== null) {
    throw new Error(
      `Reference resume server exited with code ${server.exitCode}.`,
    );
  }
...
try {
  ...
  if (
    ...
    throw new Error(
      "Reference resume timed out before completion.",
    );
  }
} finally {
  browser?.kill();
  ...
  await Promise.all([
    writeFile(
      join(
        outputRoot,
        "reference-resume-server-stdout.log",
      ),
      capturedServerStdout,
    ),
    writeFile(
      join(
        outputRoot,
        "reference-resume-server-stderr.log",
      ),
      capturedServerStderr,
    ),
    writeFile(
      join(
        outputRoot,
        "reference-resume-browser-stdout.log",
      ),
      capturedBrowserStdout,
    ),
    writeFile(
      join(
        outputRoot,
        "reference-resume-browser-stderr.log",
      ),
      capturedBrowserStderr,
    ),
    rm(
