...
const root = await mkdtemp(
  join(tmpdir(), "bgcut-strategy-bundle-"),
);
...
try {
  ...
  const process = Bun.spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        BGCUT_BROWSER_STRATEGY_BUILD_ONLY: "1",
      },
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
  ...
  if (exitCode !== 0) {
    throw new Error(
      `Browser strategy bundle check exited with code ${exitCode}.\n${stderr || stdout}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Browser strategy bundle check did not report success.\n${stdout}`,
    );
  }
