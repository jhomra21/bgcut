...
const usage =
  "Usage: bun run benchmark:browser-provider-tuning:e2e -- <manifest.json> <output-dir> <model.onnx> [runs] [timeout-ms] [port] [case-id]";
...
if (
  ...
  throw new Error(
    "The Safari provider-tuning benchmark requires macOS.",
  );
}
...
while (
  ...
  if (
    ...
    throw new Error(
      `Provider-tuning server exited with code ${server.exitCode}.`,
    );
  }
...
if (!serverReady) {
  ...
  throw new Error(
    "Provider-tuning server did not become ready.",
  );
}
...
try {
  ...
  if (!completed) {
    throw new Error(
      "Provider-tuning benchmark exceeded its launcher deadline.",
    );
  }
} finally {
  server.kill();
