...
`package.json` is the source of truth for the release version. The matching version section in `CHANGELOG.md` is the source of truth for the GitHub release body.
...
1. A push to `main` that changes `package.json`, `CHANGELOG.md`, or the release workflow itself and whose commit message starts with `chore(release):`.
2. A manual `workflow_dispatch` run when the accepted version already exists on `main` and only release metadata needs repair.

Normal product commits do not publish.
...
8. publishes through npm Trusted Publishing only when that version is missing;
...
If npm already has the exact version, the workflow skips publication and still repairs or refreshes the GitHub release metadata. This makes release repair idempotent without republishing an immutable npm version.

GitHub release notes must come from `CHANGELOG.md`. Do not use autogenerated release notes for bgcut because they can expose old internal PR titles or implementation language that is not part of the public product documentation.
...
The merge to `main` starts npm publication, registry verification, and GitHub prerelease synchronization.
...
The workflow publishes the version to npm `latest` and creates or updates a normal GitHub release.
...
If npm has the version but GitHub release creation or notes synchronization fails, do not republish or increment the version only to repair GitHub metadata. Rerun the release workflow for the same version.
